...
import (
	...
	sap_api_output_formatter "sap-api-integrations-purchase-order-creates-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	headerData, err := c.callPurchaseOrderSrvAPIRequirementHeader("A_PurchaseOrder", header)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerData, "function": "PurchaseOrderHeader"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callPurchaseOrderSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
	...
	url := strings.Join([]string{c.baseURL, "API_PurchaseOrder_SRV", api}, "/")
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_PurchaseOrder", item.PurchaseOrder)
	outputDataItem, err := c.callPurchaseOrderSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": outputDataItem, "function": "PurchaseOrderItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
